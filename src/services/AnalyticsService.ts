...
const fetchTwitter: Fetcher = async (accountId) => {
  ...
  return withRetry(async () => {
    ...
    const res = await fetch(`https://api.twitter.com/2/users/${accountId}/tweets?${params}`, {
...
const fetchInstagram: Fetcher = async (accountId) => {
  ...
  return withRetry(async () => {
    ...
    const res = await fetch(`https://graph.facebook.com/v18.0/${accountId}/media?${params}`);
...
export class AnalyticsService {
   ...
   */
  startScheduler(accountIds: Partial<Record<Platform, string>>, intervalMs = 15 * 60 * 1000): void {
  ...
  async getAggregated(
    ...
    const records = platform
      ? await this.getByPlatform(platform).then((r) =>
          r.filter((x) => x.postedAt >= from && x.postedAt <= to),
        )
